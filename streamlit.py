...
if country_cols:
    ...
    data["Country_Label"] = "Other"
    for col, label in zip(country_cols, country_labels):
        data.loc[data[col] == 1, "Country_Label"] = label

    q2 = data.groupby("Country_Label")["CO2 Emissions (Tons/Capita)"].mean().sort_values(ascending=False).head(10).reset_index()
    fig2, ax2 = plt.subplots()
    sns.barplot(data=q2, x="CO2 Emissions (Tons/Capita)", y="Country_Label", ax=ax2)
    ax2.grid(True)
    st.pyplot(fig2)
else:
    st.warning("Country columns not found in dataset.")
